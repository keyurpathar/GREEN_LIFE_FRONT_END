...
//   const [relDoc, setredDoc] = useState([])
...
//             {relDoc.slice(0, 4).map((item, indx) => (

//               <div onClick={() => navigate(`/appointment/${item._id}`)} className='border rounded-lg border-gray-300 cursor-pointer hover:-translate-y-1 transition-all duration-200'>
...
const RelatedDoctors = ({ docId, speciality }) => {
  const navigate = useNavigate()
  const {doctors} = useContext(AppContext)
  const [relDoc, setrelDoc] = useState([])   // ← also fixed typo: setredDoc → setrelDoc

  useEffect(() => {
    if (doctors.length > 0 && speciality) {
      const doctorsData = doctors.filter(
        doc => doc.speciality === speciality && doc._id !== docId  // ← key fix
      )
